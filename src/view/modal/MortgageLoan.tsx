        ...
        switch (attr) {
            case 'mortgageAmountUnit': {
                const runUpdateMortgageAmount = (() => {
                    ...
                    const _mortgageTerms = Number(state.mortgateTermsUnit.substring(0, state.mortgateTermsUnit.length-6)) * 12;
                    const _interestRate = Number(state.interestRateUnit.substring(0, state.interestRateUnit.length-2)) / 100 / 12;
                    const _monthlyRepaymentAmount = _mortgageAmount * (_interestRate * ((1 + _interestRate) ** _mortgageTerms)) / (((1 + _interestRate) ** _mortgageTerms) - 1);
                    ...
                    setState({
                        ...
                        monthlyRepaymentAmountUnit: `£ ${_monthlyRepaymentAmount.toFixed(2)}`,
            ...
            case 'mortgageTermsUnit': {
                const runUpdateMortgageTerms = (() => {
                    ...
                    const _mortgageAmount = Number(state.mortgageAmountUnit.substring(2, state.mortgageAmountUnit.length));
                    const _interestRate = Number(state.interestRateUnit.substring(0, state.interestRateUnit.length-2)) / 100 / 12;
                    const _monthlyRepaymentAmount = _mortgageAmount * (_interestRate * ((1 + _interestRate) ** (_mortgageTerms * 12))) / (((1 + _interestRate) ** (_mortgageTerms * 12)) - 1);
                    const _totalMortgageCost = _monthlyRepaymentAmount * _mortgageTerms * 12;
                    setState({
            ...
            case 'interestRateUnit': {
                const runUpdateInterestRate = (() => {
                    ...
                    const _mortgageAmount = Number(state.mortgageAmountUnit.substring(2, state.mortgageAmountUnit.length));
                    const _mortgageTerms = Number(state.mortgateTermsUnit.substring(0, state.mortgateTermsUnit.length-6)) * 12;
                    const _monthlyRepaymentAmount = _mortgageAmount * ((_interestRate/100/12) * ((1 + _interestRate/100/12) ** _mortgageTerms)) / (((1 + _interestRate/100/12) ** _mortgageTerms) - 1);
                    const _totalMortgageCost = _monthlyRepaymentAmount * _mortgageTerms;
                    setState({
                        mortgageAmountUnit: `${state.mortgageAmountUnit}`,
                        mortgateTermsUnit: `${state.mortgateTermsUnit}`,
                        ...
                        <table id="expense-tips">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" name="mortgageAmountUnit" 
                                        value={state.mortgageAmountUnit} onChange={updateMortgageRepayment}/>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                        Mortgage Terms (years)
                                    </td>
                                    <td>
                                        <input type="text" name="mortgageTermsUnit" 
                                        value={state.mortgateTermsUnit} onChange={updateMortgageRepayment}/>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                    ...
                                    <td>
                                        <input type="text" name="interestRateUnit" 
                                        value={state.interestRateUnit} onChange={updateMortgageRepayment}/>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                    ...
                                    <td>
                                        {`${state.monthlyRepaymentAmountUnit}`}                        
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                            ...
                            <thead>
                                <tr>
                                    <th colSpan={2}>
                                        <p>
                                            <b><i>
                                                For a fixed-rate mortgage, the mortgage interest rate will stay the same 
                                                until the end of the fixed rate period. When the fixed rate period comes 
                                                to an end, the mortgage will automatically move to Standard Variable Rate, 
                                                where most banks review this rate whenever the base rate changes.
                                            </i></b>
                                        </p>
                                        <ul>
                                            <li><u>See also:</u></li>
                                            <li>https://www.hsbc.co.uk/mortgages/our-rates/</li>
                                            <li>https://www.hsbc.co.uk/mortgages/bank-of-england-base-rate-calculator/</li>
                                            <li>https://www.hsbc.co.uk/help/bank-of-england-base-rate/</li>
                                        </ul>
                                    </th>
                                </tr>                                    
                            </thead>                            
