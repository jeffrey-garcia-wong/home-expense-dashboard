...
function ElectricityExpTips(props: any) {
  const { modalId } = props;
    ...
    switch (attr) {
      case "tariffPriceUnit": {
        const runUpdateTariff = () => {
          const _tariffPrice =
            val.charAt(val.length - 1) == "p"
              ? Number(val.substring(0, val.length > 1 ? val.length - 1 : 0))
              : Number(val);
          const _yearlySolarEnergy = +state.yearlySolarEnergyUnit.substring(
            0,
            state.yearlySolarEnergyUnit.length - 3,
          );
      ...
      case "dailyAvgConsumptionUnit": {
        const runUpdateDailyAvgConsumption = () => {
          const _dailyAvgConsumption =
            val.indexOf("kWh") == val.length - 3
              ? Number(val.substring(0, val.length > 3 ? +val.length - 3 : 0))
              : Number(val);
          const _yearlyAvgConsumption = _dailyAvgConsumption * 365;
          const _yearlySolarEnergy = Number(
            state.yearlySolarEnergyUnit.substring(
              0,
              state.yearlySolarEnergyUnit.length - 3,
            ),
          );
          console.log(`${attr}: ${_dailyAvgConsumption}`);
          const _tariffPrice = +state.tariffPriceUnit.substring(
            0,
            state.tariffPriceUnit.length - 1,
          );
          const _estSolarPanelCost = +state.estSolarPanelCostUnit.substring(
            1,
            state.estSolarPanelCostUnit.length,
          );
      ...
      case "yearlySolarEnergyUnit": {
        const runUpdateYearlySolarEnergy = () => {
          console.log(`${val}`);
          const _yearlySolarEnergy =
            val.indexOf("kWh") == val.length - 3
              ? Number(val.substring(0, val.length > 3 ? val.length - 3 : 0))
              : Number(val);
          const _yearlyAvgConsumption =
            +state.yearlyAvgConsumptionUnit.substring(
              0,
              state.yearlyAvgConsumptionUnit.length - 3,
            );
      ...
      case "estSolarPanelCostUnit": {
        const runUpdateSolarPanelCostUnit = () => {
          const _estSolarPanelCost =
            val.indexOf("£") == 0
              ? Number(val.substring(1, val.length))
              : Number(val);
          const _yearlyCostSaved = Number(
            state.yearlyCostSavedUnit.substring(
              1,
              state.yearlyCostSavedUnit.length,
            ),
          );
          const _offsetYears = _estSolarPanelCost / _yearlyCostSaved;
          setState({
  ...
  const yearlyCostSavingsEvaluation = () => {
    ...
    const _yearlyAvgConsumption = Number(
      state.yearlyAvgConsumptionUnit.substring(
        0,
        state.yearlyAvgConsumptionUnit.length - 3,
      ),
        ...
        <div className="row">
          <div className="col-12 col-s-12 menu">
            <table id="expense-tips">
              ...
              <tbody>
                <tr>
                  <td>Electricity price per kWh (fixed tariff)</td>
                  <td>
                    <input
                      type="text"
                      name="tariffPriceUnit"
                      value={state.tariffPriceUnit}
                ...
                <tr>
                  ...
                  <td>
                    <input
                      type="text"
                      name="dailyAvgConsumptionUnit"
                      value={state.dailyAvgConsumptionUnit}
                      onChange={updateTariff}
                    />
                  </td>
                </tr>
                <tr>
                  <td>Yearly average consumption</td>
                  <td>
                    {`${state.dailyAvgConsumptionUnit} * 365 = ${state.yearlyAvgConsumptionUnit}`}
                  </td>
                </tr>
                <tr>
                  <td>Estimated engergy generated by solar panel</td>
                  <td>
                    <input
                      type="text"
                      name="yearlySolarEnergyUnit"
                      value={state.yearlySolarEnergyUnit}
                      onChange={updateTariff}
                    />
                  </td>
                </tr>
                <tr>
                  <td>Energy cost saved by solar panel per year</td>
                  <td>
                    {`${yearlyCostSavingsEvaluation()} = ${
                      state.yearlyCostSavedUnit
                    }`}
                  </td>
                </tr>
                <tr>
                  <td>Total cost of solar panel + battery</td>
                  <td>
                    <input
                      type="text"
                      name="estSolarPanelCostUnit"
                ...
                <tr>
                  <td>Years to offset cost of solar panel + battery</td>
                  <td>
                    {`${state.estSolarPanelCostUnit} / ${state.yearlyCostSavedUnit}`}{" "}
                    <br />
                ...
                <tr>
                  <th colSpan={2}>
                    <b>
                      <i>
                        <p>
                          * if the price of electricity tariff increases, the
                          years to offset solar panel's cost reduces.
                        </p>

                        <p>
                          * if the yearly enegery consumption is lower than the
                          annual enegery generated from solar panel, the years
                          to offset solar panel's cost increase.
                        </p>
                      </i>
                    </b>
                  </th>
